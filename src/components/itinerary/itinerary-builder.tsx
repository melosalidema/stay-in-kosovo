...
export function ItineraryBuilder() {
  ...
  const googleMapsUrl = useMemo(() => {
    if (!itinerary || itinerary.stops.length < 1) {
      return null;
    }
    ...
    const origin =
      firstStop.mobility.route.points[0] ??
      firstStop.mobility.routePoints[0] ??
      firstStop.place.coordinates;

    const destination = lastStop.place.coordinates;

    const waypoints = itinerary.stops
      .slice(0, -1)
      .map((stop) => stop.place.coordinates);
  ...
  const generate = async () => {
    ...
    try {
      // Get the CSRF token required by the protected itinerary API.
      ...
      if (!response.ok || !payload.ok) {
        throw new Error(
          payload.error ?? "Failed to generate itinerary"
        );
      }
          ...
          <aside className="h-fit lg:sticky lg:top-24">
            <div className="glass-panel-strong glass-panel space-y-5 rounded-2xl p-4 sm:p-5">
              <label className="grid gap-2 text-sm font-medium">
                ...
                <Select
                  value={draft.city}
                  onValueChange={(city) => setDraft({ city })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <label className="grid gap-2 text-sm font-medium">
                ...
                <Select
                  value={draft.vibe}
                  onValueChange={(vibe) => setDraft({ vibe })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="grid gap-2 text-sm font-medium">
                ...
                <div className="flex flex-wrap gap-2">
                  {durationPresets.map((preset) => {
                    const active =
                      draft.durationHours === preset.hours &&
              ...
              <label className="grid gap-2 text-sm font-medium">
                ...
                <Select
                  ...
                  onValueChange={(transportPreference) =>
                    setDraft({
                      transportPreference:
                        transportPreference as TransportMethod
                    })
                  }
                >
                  <SelectTrigger>
          ...
          <div>
            {loading ? (
              ...
              <>
                ...
                <p className="lede mt-3 max-w-2xl text-[0.9375rem] sm:text-base">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
                  ...
                  <span>
                    {t("itinerary.days", {
                      count: itinerary.durationDays
                    })}
                  </span>

                  <span>
                    {t("itinerary.travel", {
                      minutes: itinerary.routeSummary.travelMinutes
                    })}
                  </span>
                </div>
                ...
                <ol className="mt-9">
                  {itinerary.stops.map((stop, index) => (
                    <li
                      key={stop.order}
                      className="grid grid-cols-[4.5rem_1fr] gap-x-5"
                    >
